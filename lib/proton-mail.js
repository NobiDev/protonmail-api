...
class ProtonMail {
   ...
   * @param {Browser} config.browser Puppeteer browser
   * @param {Page} config.page Puppeteer page
   * @return {ProtonMail}
   */
  ...
  async _connect () {
    if (this._browser === undefined) {
      if (this._page) {
        this._browser = this._page.browser()
      } else {
    ...
    await Promise.race([
      page.waitForSelector('#ptSidebar', { timeout: 0 }),
      page.waitForSelector('.proton-notification-template.notification-danger ', { timeout: 0 }).then(() => { invalidLogin = true })
    ])
    if (invalidLogin) {
